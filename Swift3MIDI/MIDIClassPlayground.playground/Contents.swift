...
class MIDIFrob : NSObject {
    ...
    override init() {
        ...
        if status == noErr {
            ...
            // doesn't like this
            status = MIDIInputPortCreateWithBlock(midiClient, "com.rockhoppertech.MIDIInputPort" as CFString, &inputPort, MyMIDIReadBlock)
//            if status == noErr {
...
//            }
        ...
        sequence = createMusicSequence()!
        musicPlayer = createMusicPlayer(musicSequence: sequence!)
    }
